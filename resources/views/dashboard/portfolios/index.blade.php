...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 sm:mb-8">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-white">Manajemen Portofolio</h1>
            <p class="text-slate-text text-xs sm:text-sm mt-1">Kelola portofolio acara Light Star.</p>
        </div>
        <a href="{{ route('admin.portfolios.create') }}" class="btn-cyan text-xs sm:text-sm !px-4 !py-2 w-full sm:w-auto justify-center"><span>+ Tambah Portofolio</span></a>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        @forelse($portfolios as $p)
            <div class="bg-navy-card border border-navy-border rounded-2xl overflow-hidden group min-w-0">
                ...
                <div class="p-4 sm:p-5">
                    <h3 class="text-white font-semibold text-sm sm:text-base mb-2">{{ $p->title }}</h3>
                    ...
                    @endif
                    <div class="flex flex-col sm:flex-row items-stretch gap-2 pt-4 border-t border-navy-border">
        ...
        @empty
            <div class="col-span-full text-center py-16">
                ...
                <a href="{{ route('admin.portfolios.create') }}" class="btn-cyan text-xs sm:text-sm !px-4 !py-2"><span>Tambah Portofolio
                        Pertama</span></a>
            </div>
        @endforelse
    </div>
