...
<body class="bg-navy text-white font-inter antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                <a href="{{ route('home') }}" class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-cyan rounded-xl flex items-center justify-center">
                    ...
                    <span class="text-xl font-bold">Light <span class="text-cyan">Star</span></span>
                </a>
                <nav class="flex items-center gap-6">
                    <a href="{{ route('home') }}"
                        class="text-slate-text hover:text-white transition-colors text-sm font-medium">← Kembali ke
                        Beranda</a>
                    @auth
                        @if(auth()->user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}"
                                class="btn-cyan text-sm px-5 py-2.5"><span>Dashboard</span></a>
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="btn-cyan text-sm px-5 py-2.5"><span>Sign In</span></a>
                    @endauth
                </nav>
            </div>
        </div>
    ...
    <main class="pt-32 pb-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-16 fade-up">
                ...
                <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4">Seluruh <span
                        class="text-cyan">Portofolio</span></h1>
                <p class="text-slate-text text-lg max-w-2xl">Koleksi lengkap dokumentasi dan video acara yang pernah
                            ...
                            <div class="portfolio-img-wrapper relative overflow-hidden">
                                ...
                                @if ($item->video_url)
                                    <a href="{{ $item->video_url }}" target="_blank" class="portfolio-overlay">
                                        <div
                                            class="w-12 h-12 rounded-full bg-cyan/90 backdrop-blur-sm flex items-center justify-center shadow-lg shadow-cyan/30 transition-transform duration-300 hover:scale-110">
            ...
            @else
                <div class="text-center py-24">
                    <svg class="w-16 h-16 text-slate-text/30 mx-auto mb-6" fill="none" stroke="currentColor"
                    ...
                    <h3 class="text-white text-xl font-semibold mb-2">Belum ada portofolio</h3>
                    <p class="text-slate-text">Portofolio akan segera hadir.</p>
                </div>
            @endif
        </div>
    </main>
    ...
    {{-- ═══ FOOTER ═══ --}}
    <footer class="border-t border-navy-border py-8">
